...
Fixed single-day rentals with item variations charging for a single unit. Where the per-size steppers stand in for the Quantity selector, booking two of something was priced as one and reserved one, leaving the other units still on sale. The quantity now follows the steppers, and each size is still checked against its own stock.
Fixed rentals whose Tax tab had never been touched becoming non-taxable the next time they were saved. An item with no tax status set now follows WooCommerce's own default, the tax shown in the booking summary is the tax charged at checkout, and items switched off by this are repaired automatically. An item deliberately set to Tax Status = None is left as it is.
2.7.7
...
Fixed linked shared-inventory item names displaying encoded HTML entities, and fixed the availability request referencing an undefined value when a Multiple Items rental had no configured rows.
Fixed monthly and weekly duration pricing charging a phantom extra day when hourly threshold pricing was enabled without a positive threshold or leftover hours. Rentals with a configured time-based rollover continue to use the existing threshold behavior.
Security: rental Terms & Conditions URLs are now sanitized when saved and escaped when rendered in the Classic editor, Modern editor and frontend, preventing stored script injection through malicious or legacy URL values.
