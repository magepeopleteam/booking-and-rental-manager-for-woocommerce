	...
	*/
	...
	if ( ! class_exists( 'RBFW_Resort_Function' ) ) {
		class RBFW_Resort_Function {
			...
			public function rbfw_resort_ticket_info( $product_id, $checkin_date, $checkout_date, $rbfw_room_price_category, $rbfw_room_info, $rbfw_service_info = null, $rbfw_regf_info = array() , $rbfw_room_price = array() , $rbfw_management_info = [])
            {
                global $rbfw;
                if (!empty($product_id) && !empty($checkin_date) && !empty($checkout_date) && !empty($rbfw_room_info)):
					...
					if ( $total_room_price > 0 || $service_price > 0 || $management_price > 0 ){
						$subtotal_price = (float) $total_room_price + (float) $service_price + (float) $management_price;
                    }
                    ...
                    foreach ($rbfw_fee_data as $key=> $fee) {
                        if (isset($rbfw_management_info[$fee['label']]) && $rbfw_management_info[$fee['label']] === "yes") {
                            $main_array[0]['rbfw_management_info'][ $key ] = $fee['amount'];
                        }
                    }
					...
					$main_array[0]['service_cost']    = $service_price;
					$main_array[0]['rbfw_management_price']    = $management_price;
...
<span class="price-figure" data-price="' . ( $total_price - $discount_amount + $security_deposit['security_deposit_amount'] ) . '">' . wc_price( $total_price - $discount_amount + $security_deposit['security_deposit_amount'] ) . ' ' . $tax_status . '</span></li>
            ...
            public function rbfw_get_resort_sessional_day_wise_price() {
               ...
               /* if (!(isset($_POST['nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'rbfw_ajax_action'))) {
                ...
                // Authorise the item itself — the nonce is public to every visitor.
                $post_id = rbfw_ajax_item_id( 'post_id' );

                $price         = isset( $_POST['price'] ) ? floatval( wp_unslash( $_POST['price'] ) ) : 0;
                $total_days    = isset( $_POST['total_days'] ) ? absint( wp_unslash( $_POST['total_days'] ) ) : 0;
                $checkout_date = isset( $_POST['checkout_date'] ) ? sanitize_text_field( wp_unslash( $_POST['checkout_date'] ) ) : '';
                $checkin_date  = isset( $_POST['checkin_date'] ) ? sanitize_text_field( wp_unslash( $_POST['checkin_date'] ) ) : '';
                ...
                $book_dates = (array) getAllDates( $checkin_date, $checkout_date );

                    // Bound the loop by the dates we actually resolved: total_days is
                    // client-supplied, so an inflated value must not spin the server or
                    // read past the end of the array.
                    $total_days = min( $total_days, count( $book_dates ) );
