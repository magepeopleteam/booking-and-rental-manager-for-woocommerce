           ...
           /*     if ( ! empty( $rent_info ) ) {
			...
			public function rbfw_bikecarsd_ticket_info( $product_id, $rbfw_start_datetime = null, $rbfw_end_date = null, $rbfw_type_info = array(), $rbfw_service_info = array(), $selected_time = null, $rbfw_regf_info = array(), $rbfw_pickup_point = null, $rbfw_dropoff_point = null, $end_time = null, $rbfw_item_quantity = null , $booking_date = null) {
				...
				if ( ! empty( $product_id ) && ! empty( $rbfw_type_info ) ):
					...
					$main_array[0]['rbfw_end_date']           = $rbfw_end_date;
			...
			public function rbfw_bikecarsd_ajax_price_calculation() {
				...
				$content .= '<div class="item rbfw_bikecarsd_price_summary">
                                <div class="item-content rbfw-costing">
                                    <ul class="rbfw-ul">
                                        <li class="duration-costing rbfw-cond">' . esc_html__( 'Duration Cost', 'booking-and-rental-manager-for-woocommerce' ) . ' <span class="price-figure" data-price="' . $total_bikecarsd_price . '">' . wc_price( $total_bikecarsd_price ) . '</span></li>';
				if ( ! empty( $service_price_arr ) ) {
					$content .= '<li class="resource-costing rbfw-cond">' . esc_html__( 'Resource Cost', 'booking-and-rental-manager-for-woocommerce' ) . ' <span class="price-figure" data-price="' . $total_service_price . '">' . wc_price( $total_service_price ) . '</span></li>';
				}
				$content .= '<li class="subtotal">' . esc_html__( 'Subtotal', 'booking-and-rental-manager-for-woocommerce' ) . '<span class="price-figure" data-price="' . $subtotal_price . '">' . wc_price( $subtotal_price ) . '</span></li>';
				...
				$total_price = $total_price + $security_deposit['security_deposit_amount'];
				$content     .= '<li class="total"><strong>' . esc_html__( 'Total', 'booking-and-rental-manager-for-woocommerce' ) . '</strong> <span class="price-figure" data-price="' . $total_price . '">' . wc_price( $total_price ) . ' ' . $tax_status . '</span></li>
