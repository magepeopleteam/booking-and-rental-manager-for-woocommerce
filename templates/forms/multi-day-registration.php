...
jQuery(document).ready(function($) {
	console.log('RBFW WooCommerce Products JS loaded inline (multi-day)');
	...
	function updateBookingSubtotal() {
		var durationCost = parseFloat($('.duration-costing .price-figure').data('price') || 0);
		var resourceCost = parseFloat($('.resource-costing .price-figure').data('price') || 0);
		...
		$('.subtotal .price-figure').html('€' + subtotal.toFixed(2)).data('price', subtotal);
		
		// Update total
		var securityDeposit = parseFloat($('.security_deposit .price-figure').data('price') || 0);
		var total = subtotal + securityDeposit;
		
		$('.total .price-figure').html('€' + total.toFixed(2)).data('price', total);
	}
