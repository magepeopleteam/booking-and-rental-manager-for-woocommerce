...
<tr class="rbfw_bike_car_sd_price_table_row" data-key="">
    ...
    <td>
        <input class="medium" type="number" name="rbfw_bike_car_sd_data[<?php echo $total_row ?>][price]" step=".01" value="" placeholder="Price">
    </td>
    ...
    <td class="rbfw_time_inventory rbfw_time_inventory_enable duration_disable <?php echo ($manage_inventory_as_timely=='off')?'rbfw_hide':(($manage_inventory_as_timely=='on' && $enable_specific_duration =='on')?'rbfw_hide':'')  ?>">
        <select class="medium" name="rbfw_bike_car_sd_data[<?php echo $total_row ?>][d_type]">
            <option>Hours</option>
            <option>Days</option>
            <option>Weeks</option>
        </select>
