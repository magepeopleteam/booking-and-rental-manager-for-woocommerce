...
<tr class="rbfw_bike_car_sd_price_table_row" data-key="">
    <td>
        <input  type="text" name="rbfw_bike_car_sd_data[<?php echo esc_html($total_row); ?>][rent_type]" value="" placeholder="Type name">
    </td>
    <td>
        <input  type="text" name="rbfw_bike_car_sd_data[<?php echo esc_html($total_row); ?>][short_desc]" value="" step=".01" placeholder="Short Description">
    </td>
    <td>
        <input class="medium" type="number" name="rbfw_bike_car_sd_data[<?php echo esc_html($total_row); ?>][price]" step=".01" value="" placeholder="Price">
    </td>

    <td class="rbfw_without_time_inventory  <?php echo esc_html(($manage_inventory_as_timely=='on') ? 'rbfw_hide' : ''); ?>">
        <input class="medium" type="number" name="rbfw_bike_car_sd_data[<?php echo esc_html($total_row); ?>][qty]"  placeholder="<?php esc_html_e('Stock Quantity','booking-and-rental-manager-for-woocommerce'); ?>">
    </td>
    ...
    <td class="rbfw_time_inventory rbfw_time_inventory_enable duration_disable <?php echo esc_html(($manage_inventory_as_timely=='off')?'rbfw_hide':(($manage_inventory_as_timely=='on' && $enable_specific_duration =='on')?'rbfw_hide':''));  ?>">
        <input class="medium" type="number" name="rbfw_bike_car_sd_data[<?php echo esc_html($total_row); ?>][duration]" value="" placeholder="Duration">
    </td>

    <td class="rbfw_time_inventory rbfw_time_inventory_enable duration_disable <?php echo esc_html(($manage_inventory_as_timely=='off')?'rbfw_hide':(($manage_inventory_as_timely=='on' && $enable_specific_duration =='on')?'rbfw_hide':''));  ?>">
        <select class="medium" name="rbfw_bike_car_sd_data[<?php echo esc_html($total_row); ?>][d_type]">
            <option>Hours</option>
            <option>Days</option>
            <option>Weeks</option>
        </select>
