...
let hourlyPriceEnabled = false;
let hourThresholdEnabled = false;
...
function generateTimeSlots() {
    ...
    const endHour = 21; // Ending at 9:00 PM
...
function addTimeSlot() {
    ...
    if (timeSlots.some(slot => slot.time === newSlotTime)) {
        alert('This time slot already exists!');
        return;
    }

    const maxId = Math.max(...timeSlots.map(slot => slot.id), 0);
    const newSlot = {
        id: maxId + 1,
        time: newSlotTime,
        enabled: true
    ...
    // Add the slot and sort all slots by time
...
newSlotTimeInput.addEventListener('input', function() {
    addSlotBtn.disabled = !this.value;
});
