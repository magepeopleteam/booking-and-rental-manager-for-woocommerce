	...
	if ( ! class_exists( 'Rbfw_Account_Page' ) ) {
		class Rbfw_Account_Page {
			...
			public function rbfw_ba_user_signin_signup_form_submit() {
				...
				$request          = isset( $_POST['rbfw_ba_user_submit_request'] ) ? sanitize_text_field( wp_unslash($_POST['rbfw_ba_user_submit_request'] )): '';
				$email            = isset( $_POST['rbfw_ba_user_email'] ) ? sanitize_email( wp_unslash($_POST['rbfw_ba_user_email'] )) : '';
				$password         = isset( $_POST['rbfw_ba_user_password'] ) ? wp_strip_all_tags( wp_unslash($_POST['rbfw_ba_user_password'] )) : '';
				$confirm_password = isset( $_POST['rbfw_ba_user_cpassword'] ) ? sanitize_text_field( wp_unslash($_POST['rbfw_ba_user_cpassword'] )) : '';
				$first_name       = isset( $_POST['rbfw_ba_user_fname'] ) ? sanitize_text_field( wp_unslash($_POST['rbfw_ba_user_fname'] )) : '';
				$last_name        = isset( $_POST['rbfw_ba_user_lname'] ) ? sanitize_text_field( wp_unslash($_POST['rbfw_ba_user_lname'] )) : '';
				if ( $request == 'signin' ) {
					$errors = '';
					if ( empty( $email ) ):
			...
			public function rbfw_account_view_order_details() {
				...
				$current_user_email = $current_user->user_email;
				if ( isset( $_GET['view_order'] ) ) {
					$order_id      = sanitize_text_field( wp_unslash($_GET['view_order'] ));
