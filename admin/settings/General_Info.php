	...
	if (!class_exists('RBFW_General_Info')) {
        class RBFW_General_Info{
                ...
                <section>
                    <div>
                        <label>
                            ...
                            printf(
                                '%s %s %s',
                                esc_html__( 'Select', 'booking-and-rental-manager-for-woocommerce' ),
                                esc_html( $label ),
                    ...
                    <div class="w-50">
                        <select name="rbfw_categories[]" multiple class="category2">
                            ...
                            foreach ( $terms as $key => $value ) {
                                ?>
                                <option <?php echo esc_attr(in_array($value->name, $rbfw_categories) ? 'selected' : ''); ?> value="<?php echo esc_attr($value->name); ?>"> 
                ...
                <div id="field-wrapper-<?php echo esc_attr( $id ); ?>" class="field-wrapper field-text-multi-wrapper field-text-multi-wrapper-<?php echo esc_attr( $field_id ); ?>">
                    <section>
                        <div class="w-100">
                                                                ...
                                                                <div class="feature_category_inner_item_wrap sortable">
                                                                    ...
                                                                    if ( ! empty( $value['cat_features'] ) ) {
                                                                        ...
                                                                        foreach ( $value['cat_features'] as $feature ) {
                                                                            ...
                                                                            ?>
                                                                            <div class="item">
                                                                                <a href="#rbfw_features_icon_list_wrapper" class="rbfw_feature_icon_btn btn" data-key="<?php echo esc_attr( $c ); ?>">
                                                                                    <i class="fas fa-circle-plus"></i> <?php echo esc_html__( 'Add Icon', 'booking-and-rental-manager-for-woocommerce' ); ?>
                                                                                </a>
                                                                                <div class="rbfw_feature_icon_preview" data-key="<?php echo esc_attr( $c ); ?>">
                                                                                ...
                                                                                <span class="button remove" onclick="jQuery(this).parent().parent().remove()">
                                                                                    <?php echo wp_kses($remove_text, array('i' => array('class' => array()))); ?>
                                                                                </span>
                                                                            </div>
                                                                            <?php
                                                                            $c++;
                                            ...
                                            <div class="features_category_wrapper text-center">
                                                <div class="field-list <?php echo esc_attr( $field_id ); ?>">
                                                    <div class="feature_category_title">
                                                        ...
                                                        <input type="text" name="rbfw_feature_category[0][cat_title]" data-key="0" placeholder="<?php echo esc_attr__( 'Feature Category Label', 'booking-and-rental-manager-for-woocommerce' ); ?>"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                            ...
                            <span class="ppof-button add-feature-category mt-1">
                                <i class="fas fa-circle-plus"></i> 
                                <?php echo esc_html__( 'Add New Feature Category', 'booking-and-rental-manager-for-woocommerce' ); ?>
                            </span>
                        </div>
                    </section>
                </div>
            ...
            public function features_category( $post_id ) {
                ...
                $field_name = isset( $options['field_name'] ) ? esc_attr( $options['field_name'] ) : $id;
                ...
                $remove_text = isset( $options['remove_text'] ) ? wp_kses_post( $options['remove_text'] ) : '<i class="fas fa-trash-can"></i>';
                ...
                $limit = !empty( $options['limit'] ) ? esc_attr( $options['limit'] ) : '';
            
                $field_id = esc_attr( $id );
                $field_name = !empty( $field_name ) ? esc_attr( $field_name ) : $id;
                ...
                <script type="text/javascript">
                    ...
                    jQuery(document).on('click', '.add-new-feature',function(e){
                        ...
                        html += '<input type="hidden" name="rbfw_feature_category['+ dataCat +'][cat_features]['+ c +'][icon]" placeholder="<?php echo esc_html__('Icon','booking-and-rental-manager-for-woocommerce'); ?>" data-key="'+ c +'" class="rbfw_feature_icon"/>';
                        html += '<input type="text" name="rbfw_feature_category['+ dataCat +'][cat_features]['+ c +'][title]" placeholder="<?php echo esc_attr($placeholder); ?>" data-key="'+ c +'"/>';
                        html += '<div>';
                        <?php if($sortable):?>
                        html += ' <span class="button sort" ><i class="fas fa-arrows-alt"></i></span>';
                        <?php endif; ?>
                        html += '<span class="button remove" onclick="jQuery(this).parent().parent().remove()"><?php echo wp_kses_post($remove_text); ?></span>';
                        html += '</div>';
                        html += '</div>';
